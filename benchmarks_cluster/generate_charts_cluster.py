#!/usr/bin/env python3
"""
Genera grafici di confronto benchmark dell'esecuzione su cluster AWS EMR.
Legge benchmarks_cluster/benchmarks_cluster.csv e produce 3 grafici in
benchmarks_cluster/charts_cluster/.

Uso: python3 benchmarks_cluster/generate_charts_cluster.py
"""
...
CSV_PATH = os.path.join(SCRIPT_DIR, "benchmarks_cluster.csv")
CHARTS_DIR = os.path.join(SCRIPT_DIR, "charts_cluster")
os.makedirs(CHARTS_DIR, exist_ok=True)

if not os.path.exists(CSV_PATH):
    raise SystemExit(
        f"ERRORE: {CSV_PATH} non trovato. "
        "Esegui prima `bash cluster/download_results.sh <bucket>` per scaricarlo."
    )
...
fig.suptitle('Scalabilità su cluster AWS EMR: tempo di esecuzione vs dimensione dataset',
...
fig.suptitle('Confronto tecnologie al 100% del dataset (cluster AWS EMR)',
...
fig.suptitle('Heatmap tempi di esecuzione su cluster AWS EMR (secondi)',
             fontsize=13, fontweight='bold')
...
print("\nGrafici cluster generati in benchmarks_cluster/charts_cluster/")
