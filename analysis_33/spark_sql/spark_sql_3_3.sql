with aeroporto_stat as(
    select origin,
           cast(round((avg(case when cancelled = 0 then dep_delay end)), 2) as decimal(10,2)) as ritardo_medio_complessivo
...
aeroporto_compagnia_stat as (
    ...
    cast(round(avg(case when cancelled = 0 then dep_delay end), 2) as decimal(10,2)) as ritardo_medio_partenza,
    cast(round(avg(case when cancelled = 0 then arr_delay end), 2) as decimal(10,2)) as ritardo_medio_arrivo,
    cast(round((1.0* sum(case when cancelled = 1 then 1.0 else 0.0 end)) / (1.0*count(*)) * 100.0, 1) as decimal(10,1)) as tasso_cancellazione
    from flights_project.flights
...
select ac.aeroporto_partenza,
       ...
       cast(round((ac.ritardo_medio_partenza - a.ritardo_medio_complessivo), 2) as decimal(10,2)) as differenza,
